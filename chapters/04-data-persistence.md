...
この章では、アプリのデータを端末に永続的に保存する方法として、SwiftDataとAppStorageの使い分けを学ぶ。

SwiftDataではメモのような構造化データを保存し、@QueryやmodelContextを使って自動的に一覧表示・更新を行う。

一方でAppStorageではユーザー名や表示設定などの小さな設定値を簡単に保存する方法を学ぶ。
...
1. **質問：**
   ...
   もし @Query を使わずに @State で memos を管理した場合、何が問題になりますか？
   
   **得られた理解：**

   @State はデータベースとつながっていないため、データの変更を自動で検知できない。そのため、追加や削除をしても画面が自動更新されず、表示と実際のデータがずれる可能性がある。

2. **質問：**

   SwiftData はなぜ insert / delete だけで保存できて、save() が不要なのですか？
   
   **得られた理解：**

   modelContext がデータの変更を自動で監視しており、変更が発生したタイミングで自動的に永続化される仕組みになっている。そのため手動で save() を呼ぶ必要がない

3. **質問：**

   なぜユーザー設定（@AppStorage）とメモ（SwiftData）を同じ方法で保存しないのですか？
   
   **得られた理解：**

   データの性質が異なるためである。ユーザー設定は名前や表示設定のような単純な値で、数も少なく頻繁に構造が変わらないため @AppStorage が適している。

   一方、メモはタイトル・内容・作成日時など複数の情報を持つデータで、追加や削除も行うため SwiftData のようなデータベース管理が必要になる。

## この章のまとめ

（この章で学んだ最も重要なことを、未来の自分が読み返したときに役立つように書く）

この章の最も重要なポイントは、「データの種類によって保存方法を使い分ける」という考え方である。

メモのように増減や編集があるデータはSwiftData、設定のような単純な値はAppStorageを使うことで、効率的に永続化できる。

また、SwiftUIでは@QueryやmodelContextによってデータ変更が自動で画面に反映されることも理解した。
